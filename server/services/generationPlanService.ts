...
import { GenerationPlan, HourlyPlan, Unit, GridLoadForecast } from '../../shared/types';

const MAX_RAMP_RATE = 50;
const MIN_FUEL_LIFE_FOR_FULL_POWER = 20;

export const generateGenerationPlan = async (
  unitId: string,
  date: string,
  gridForecast?: number[]
): Promise<GenerationPlan> => {
  const unit = await getQuery('SELECT * FROM units WHERE id=?', [unitId]) as Unit;
  if (!unit) {
  ...
  if (gridForecast) {
    ...
    const forecast = await getQuery('SELECT * FROM grid_load_forecast WHERE date=?', [date]);
    if (forecast) {
  ...
  const allUnits = await allQuery('SELECT * FROM units WHERE status=?', ['operational']);
...
export const validatePlanAdjustment = async (
  planId: string,
  newHourlyPlan: HourlyPlan[]
): Promise<{ valid: boolean; issues: string[] }> => {
  const issues: string[] = [];
  const plan = await getQuery('SELECT * FROM generation_plans WHERE id=?', [planId]);
  if (!plan) {
    return { valid: false, issues: ['Plan not found'] };
  }

  const unit = await getQuery('SELECT * FROM units WHERE id=?', [plan.unitId]);
  if (!unit) {
