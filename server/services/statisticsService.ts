...
import { Statistics, Unit, RealtimeData, RadiationReading, Equipment, GenerationPlan } from '../../shared/types';
...
export const calculateStatistics = async (
  ...
  const units = unitId === 'all' 
    ? await allQuery('SELECT * FROM units') 
    : [await getQuery('SELECT * FROM units WHERE id=?', [unitId])];
  ...
  for (const unit of units) {
    ...
    const realtimeData = await allQuery(
      'SELECT * FROM realtime_data WHERE unitId=? AND timestamp >= ? AND timestamp <= ? ORDER BY timestamp',
      [unit.id, startDate, endDate]
    ) as RealtimeData[];

    const radiationData = await allQuery(
      'SELECT * FROM radiation_readings WHERE timestamp >= ? AND timestamp <= ? ORDER BY timestamp',
      [startDate, endDate]
    ) as RadiationReading[];

    const equipmentList = await allQuery(
      'SELECT * FROM equipment WHERE unitId=?',
      [unit.id]
    ) as Equipment[];

    const plans = await allQuery(
      'SELECT * FROM generation_plans WHERE unitId=? AND date >= ? AND date <= ?',
      [unit.id, startDate, endDate]
    );
...
export const exportToPDF = async (
  ...
  const statistics = await calculateStatistics(unitId, period, startDate, endDate);
  const units = await allQuery('SELECT * FROM units');
  ...
  for (const stat of statistics) {
    const unit = units.find((u: any) => u.id === stat.unitId);
  ...
  const equipment = await allQuery('SELECT * FROM equipment');
