...
import { RealtimeData, Alarm } from '../../shared/types';
...
export const checkAlarms = async (data: RealtimeData): Promise<Alarm | null> => {
  const checks: Array<{ type: keyof typeof THRESHOLDS; value: number }> = [
...
const getAlarmMessage = (type: string, level: string, value: number, threshold: number): string => {
...
export const executeProtectionAction = async (unitId: string, alarm: Alarm): Promise<{ action: string; newControlRodPosition: number }> => {
  let action = '';
  ...
  await runQuery(
    'UPDATE alarms SET autoActionTaken=? WHERE id=?',
    [action, alarm.id]
  );
