...
const seedData = async () => {
    const existingUnits = await (0, database_1.getQuery)('SELECT COUNT(*) as count FROM units');
    ...
    for (const emp of employees) {
        await (0, database_1.runQuery)('INSERT INTO employees (id, name, employeeNo, skills, maxHoursPerWeek, currentHoursThisWeek, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [emp.id, emp.name, emp.employeeNo, JSON.stringify(emp.skills), emp.maxHoursPerWeek, emp.currentHoursThisWeek, emp.status]);
    }
    const equipmentData = [
    ...
    for (const eq of equipmentData) {
        await (0, database_1.runQuery)('INSERT INTO equipment (id, name, unitId, category, criticalityLevel, operatingHours, lastMaintenance, nextMaintenance, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [eq.id, eq.name, eq.unitId, eq.category, eq.criticalityLevel, eq.operatingHours, eq.lastMaintenance, eq.nextMaintenance, eq.status]);
    }
    ...
    for (const item of inventoryItems) {
        await (0, database_1.runQuery)('INSERT INTO inventory (id, partName, partNo, quantity, minStock, maxStock, location, lastUpdated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [item.id, item.partName, item.partNo, item.quantity, item.minStock, item.maxStock, item.location, item.lastUpdated]);
    }
    ...
    for (let i = 0; i < 7; i++) {
        ...
        await (0, database_1.runQuery)('INSERT INTO grid_load_forecast (id, date, hourlyLoad) VALUES (?, ?, ?)', [`forecast_${i}`, dateStr, JSON.stringify(hourlyLoad)]);
    }
    const users = [
    ...
    for (const user of users) {
        await (0, database_1.runQuery)('INSERT INTO users (id, username, role, name) VALUES (?, ?, ?, ?)', [user.id, user.username, user.role, user.name]);
    }
